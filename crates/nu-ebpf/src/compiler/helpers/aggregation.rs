...
//! - bpf-count (supports both integer and string keys)
//! - bpf-histogram

use nu_protocol::RegId;

use crate::compiler::CompileError;
use crate::compiler::elf::{BpfFieldType, MapRelocation};
use crate::compiler::instruction::{BpfHelper, EbpfInsn, EbpfReg, opcode};
use crate::compiler::ir_to_ebpf::{
    COUNTER_MAP_NAME, HISTOGRAM_MAP_NAME, IrToEbpfCompiler, STRING_COUNTER_MAP_NAME,
};
...
impl AggregationHelpers for IrToEbpfCompiler<'_> {
    ...
    /// - Integer keys (8 bytes): for numeric values like $ctx.pid
    /// - String keys (16 bytes): for $ctx.comm (process name)
    fn compile_bpf_count(&mut self, src_dst: RegId) -> Result<(), CompileError> {
        // Check if we have a string key (Comm type)
        let is_string_key = matches!(self.get_register_type(src_dst), Some(BpfFieldType::Comm));

        if is_string_key {
            self.compile_string_count(src_dst)
        } else {
            self.compile_int_count(src_dst)
        }
    }
...
/// Private helper methods for count implementations
impl IrToEbpfCompiler<'_> {
    /// Compile count with integer key (8 bytes)
    pub(crate) fn compile_int_count(&mut self, src_dst: RegId) -> Result<(), CompileError> {
    ...
    /// Compile count with string key (16 bytes, for $ctx.comm)
    ///
    /// Uses the stack-based string that was already read for $ctx.comm
    pub(crate) fn compile_string_count(&mut self, src_dst: RegId) -> Result<(), CompileError> {
        self.set_needs_string_counter_map(true);

        // Get the stack string info for the comm
        let stack_str = self.get_stack_string(src_dst).ok_or_else(|| {
            CompileError::UnsupportedInstruction(
                "String count requires a stack-based string (like $ctx.comm)".into(),
            )
        })?;

        let key_stack_offset = stack_str.offset;

        // Allocate stack space for value only (key is already on stack from comm read)
        // Value: 8 bytes (i64)
        let value_stack_offset = self.alloc_stack(8)?;
        ...
        self.add_relocation(MapRelocation {
            insn_offset: lookup_reloc_offset,
            map_name: STRING_COUNTER_MAP_NAME.to_string(),
        });

        // R2 = pointer to key (the 16-byte comm string on stack)
        self.builder()
            .push(EbpfInsn::mov64_reg(EbpfReg::R2, EbpfReg::R10));
        self.builder()
            .push(EbpfInsn::add64_imm(EbpfReg::R2, key_stack_offset as i32));
